...
{
  // get track, process name, scintillator id, particle name
  G4Track* aTrack = aStep->GetTrack();
  G4String processName = aStep->GetPostStepPoint()->GetProcessDefinedStep()->GetProcessName();
  G4int id = aStep->GetPreStepPoint()->GetTouchableHandle()->GetCopyNumber(0);	//CopyNumber of the detector volume
